...
class ProductViewHelper
{
    ...
    public static function displaySingleProduct(Videogames $videogames): string
    {
        $output = '<div>';
        $output .= "<h1>$videogames->name</h1>";
        $output .= "<p>$videogames->id</p>";
        $output .= "<p>$videogames->release_year</p>";
        $output .= "<p>$videogames->platform_id</p>";
        $output .= '</div>';
    ...
    public static function displayAllVideogames(array $videogames): string
    {
        ...
        foreach ($videogames as $videogame) {
            ...
            $output .= "<p>$videogame->id</p>";
            $output .= "<p>$videogame->release_year</p>";
            $output .= "<p>$videogame->platform_id</p>";
            $output .= '</div>';
        }
